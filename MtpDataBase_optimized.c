			...
			void *server)
{
	...
	/* is Root Dir? - 🚀 CRITICAL OPTIMIZATION HERE! */
	if (control == MTP_TOOLS_FUNCTION_UPDATE &&
		!strcmp(storage->mFilePath, buf)) {
		
		printf("🚀 Root directory update detected - using FAST method (avoiding recursive scan)\n");
		
		// ❌ 注释掉耗时的递归扫描！这是30秒延迟的根源！
		...
		printf("🚀 Sending fast root update notification (handle: %u)\n", rootHandle);
		sendObjectInfoChanged(rootHandle, mServer);
		
		// 🚀 额外发送存储变更事件，确保PC端刷新
		printf("🚀 Sending storage info changed event (storageID: %u)\n", storageID);
		MtpServerSendEvent(MTP_EVENT_STORE_INFO_CHANGED, storageID, mServer);
		
		printf("🚀 Fast root update completed in milliseconds instead of 30+ seconds!\n");
		return 0;
	}
