...
#define OPERATION_QUIET_TIME 1  // 减少到1秒，提高响应速度
#define EVENT_CHECK_INTERVAL 500  // 减少到500ms，更频繁检查
#define MAX_EVENT_WAIT_TIME 5    // 减少最大等待时间到5秒
#define DELETION_IMMEDIATE_SEND 1  // 删除操作立即发送标志
...
static int is_operation_start_event(struct inotify_event *event) {
    return (event->mask & (IN_CREATE | IN_MOVED_FROM | IN_DELETE)) && 
           (event->mask & IN_ISDIR);
}
...
static void mark_deletion_operation(const char *path) {
    ...
    operation_state.immediate_send = 1;
...
static void mark_operation_start(const char *path) {
    ...
    if (operation_state.state == OP_STATE_IDLE) {
        ...
        operation_state.immediate_send = 0;
...
static int check_operation_complete(char *update_path, size_t path_size, int *is_deletion) {
    ...
    *is_deletion = operation_state.is_deletion;
    ...
    if (operation_state.state != OP_STATE_IDLE) {
        // 删除操作立即发送
        if (operation_state.state == OP_STATE_DELETION && operation_state.immediate_send) {
            should_send = 1;
        ...
        // 检查操作是否真正结束
        else if (operation_state.state == OP_STATE_ENDING) {
            // 如果静默时间内没有新事件，认为操作完成
        ...
        // 检查操作是否长时间无活动
        else if ((now - operation_state.last_event_time) >= OPERATION_QUIET_TIME) {
            operation_state.state = OP_STATE_COMPLETED;
            should_send = 1;
            strncpy(update_path, operation_state.path, path_size - 1);
            update_path[path_size - 1] = '\0';
...
static void remove_watch_entries_by_path_prefix(const char *path_prefix) {
    ...
    while (cur) {
        if (strncmp(cur->path, path_prefix, prefix_len) == 0 && 
            ...
            inotify_rm_watch(-1, cur->wd); // 这里inotify_fd会在调用处传入
...
static void handle_inotify_event(int inotify_fd, struct inotify_event *event) {
    ...
    if (is_deletion_event(event)) {
        ...
        if (event->mask & IN_ISDIR) {
            remove_watch_entries_by_path_prefix(full_path);
        }
