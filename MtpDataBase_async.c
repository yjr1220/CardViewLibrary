	...
	case MTP_TYPE_UINT16:
		{
			...
			DLOG("value:0x%x", value);
			mData->putData16(value, mData);
		}
		break;
	default:
		DLOG("!!!!! FIXME handle other type value 0x%x", type);
		break;
	}
...
// 🚀 完整的目录扫描函数（保持原有逻辑，不限制深度）
static void MtpToolsCommandUpdateDirComplete(struct Dir *dir, struct list_head *objectList,
                struct MtpStorage *storage, struct MtpServer *mServer)
{
	...
	DLOG("objectChildNum = %zu, scandir num = %d", objectChildNum, num);
	...
	for (i = 0; i < num; i++) {
		...
		if (namelist[i]->d_type & DT_DIR) {
			...
			// 🚀 递归扫描所有子目录（不限制深度）
			MtpToolsCommandUpdateDirComplete(objectInfo->object.dir, objectList, storage, mServer);
...
void* background_scan_thread(void* arg) {
    struct async_scan_data *data = (struct async_scan_data*)arg;
    
    printf("🚀 Background scan started for: %s\n", data->dir_path);
    
    // 执行完整的目录扫描（不限制深度）
    MtpToolsCommandUpdateDirComplete(data->dir, data->objectList, data->storage, data->mServer);
    
    // 扫描完成后，再次发送根目录变更通知
    MtpObjectHandle rootHandle = data->dir->object ? 
        ((struct MtpObjectInfo *)data->dir->object)->mHandle : MTP_PARENT_ROOT;
    sendObjectInfoChanged(rootHandle, data->mServer);
    
    printf("🚀 Background scan completed for: %s\n", data->dir_path);
    free(data);
    return NULL;
}
...
{
    // 对于非根目录，直接使用完整扫描
    MtpToolsCommandUpdateDirComplete(dir, objectList, storage, mServer);
}

// 🚀 ASYNC OPTIMIZED VERSION - 异步扫描优化版本
			...
			void *server)
{
	...
	/* is Root Dir? - 🚀 ASYNC OPTIMIZATION */
	if (control == MTP_TOOLS_FUNCTION_UPDATE &&
		!strcmp(storage->mFilePath, buf)) {
		
		printf("🚀 Root directory update detected - using ASYNC method\n");
		...
		printf("🚀 Sending immediate notification (handle: %u)\n", rootHandle);
		sendObjectInfoChanged(rootHandle, mServer);
		
		// 2. 启动后台完整扫描线程
		struct async_scan_data *data = malloc(sizeof(struct async_scan_data));
		if (data) {
			data->dir = storage->mDisk->dDirRoot;
			data->objectList = objectList;
			data->storage = storage;
			data->mServer = mServer;
			data->storageID = storageID;
			strncpy(data->dir_path, storage->mFilePath, PATH_MAX - 1);
			data->dir_path[PATH_MAX - 1] = '\0';
			
			pthread_t scan_thread;
			if (pthread_create(&scan_thread, NULL, background_scan_thread, data) == 0) {
				pthread_detach(scan_thread);
				printf("🚀 Background scan thread started successfully\n");
			} else {
				printf("❌ Failed to create background scan thread, doing sync scan\n");
				MtpToolsCommandUpdateDirComplete(storage->mDisk->dDirRoot, objectList, storage, mServer);
				free(data);
			}
		} else {
			printf("❌ Failed to allocate memory for async scan, doing sync scan\n");
			MtpToolsCommandUpdateDirComplete(storage->mDisk->dDirRoot, objectList, storage, mServer);
		}
		
		printf("🚀 Async root update initiated - returning immediately\n");
		return 0;  // 立即返回，不阻塞
	}
